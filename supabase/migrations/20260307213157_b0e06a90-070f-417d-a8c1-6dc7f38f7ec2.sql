...
CREATE POLICY "HR users can create jobs" ON public.jobs
  ...
  WITH CHECK (EXISTS (SELECT 1 FROM profiles WHERE profiles.user_id = auth.uid() AND profiles.role = 'hr'));
...
-- APPLICATIONS
CREATE POLICY "Applicants can view own or HR can view all" ON public.applications
  FOR SELECT TO authenticated
  USING (applicant_id = auth.uid() OR EXISTS (SELECT 1 FROM profiles WHERE profiles.user_id = auth.uid() AND profiles.role = 'hr'));
...
CREATE POLICY "HR can update applications" ON public.applications
  ...
  USING (EXISTS (SELECT 1 FROM profiles WHERE profiles.user_id = auth.uid() AND profiles.role = 'hr'));
